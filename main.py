...
#     mycursor.execute ("""SELECT * FROM customers WHERE MobileNum='{}'""".format(addMobileNum))
#     customers = mycursor.fetchall()
#     if len(customers)>0:
#         return render_template('register.html', message='Entered mobile number was already registered. Login to proceed')

#     mycursor.execute("""insert into `customers` (`CustomerName`, `MobileNum`, `Email`, `Password`) values
#              ('{}', '{}', '{}', '{}')""".format(addName, addMobileNum, addEmail, addPassword))
#     mydb.commit()
...
def admininfo():
  ...
  # mycursor.execute("""select * from `orders`  order by `Date` desc""")
  # orderstaken = mycursor.fetchall()
  # orderdetails={}
  # statusreport={}
  # for i in orderstaken:
  ...
  # return render_template('admininfo.html',empinfo=empinfo,reservations=reservations,orderstaken=orderstaken,orderdetails=orderdetails,statusreport=statusreport)
...
def place_order():
  ...
  if payment_method != None:
    ...
    for dish in cartDishes:
      ...
      # mycursor.execute("""insert into `orderDetails` (`DetailsId`, `DishId`, `Quantity`) values ('{}', '{}', '{}')""".format(addDetailsId, dish[0], dish_quantity))
      mydb.commit()
      ############
      # check delivery availabilty
      ############
    ...
    # assignedAgent = mycursor.fetchall()
    # if len(assignedAgent)==0:
    #   return render_template('orderFinished.html', message="Sorry, all of our deliver agents are busy right now..Please try again after some time", message_code="Failed")
...
def payment_otp_authentication():
  ...
  if entered_otp != payment_otp : 
    ...
    # return render_template('wallet.html',message="Valid OTP")
    return redirect('/place_order_continued')
...
#   # print("hello")
#   if 'CustomerId' in session:
#     mycursor.execute("""select * from `orders`  where  `CustomerId`= '{}'""".format(session['CustomerId']))
#     orderstaken = mycursor.fetchall()
#     orderdetails={}
#     for i in orderstaken:
#       mycursor.execute("""select `menu`.`DishName`,`orderDetails`.`Quantity`,(`menu`.`Price`)*(`orderDetails`.`Quantity`) from (`orderDetails` inner join `menu` on `orderDetails`.`DishId`=`menu`.`DishID` )  where  `orderDetails`.`DetailsId`= '{}'""".format(i[4]))
...
#     # if (orderstaken==[]):
#       # Noorder = True
#     return render_template('myOrders.html', orderstaken=orderstaken,orderdetails=orderdetails)
#   else:
...
#   where tables.TableId not in (select TableId from dailyreservation where Slot = timeslots.Slot) and tables.Capacity='{}';""".format(capacity))
#   availability = mycursor.fetchall()
#   print(availability)
#   slot_table = {}
#   for pair in availability:
#     slot_table[pair[1]] = pair[0]
#   return render_template('reservations.html', slot_table=slot_table)
...
def check():
  ...
  global select_date
  select_date=date
  mycursor.execute("""select tables.TableId, timeslots.Slot from (tables cross join timeslots) where tables.TableId not in (select TableId from Reservations where Slot = timeslots.Slot and Date='{}') and tables.Capacity='{}' ;""".format(date,capacity))
  availability = mycursor.fetchall()
  print(availability)
  mycursor.execute("""SELECT DATE_ADD( CURRENT_DATE(), interval 1 day)""")
  tomdate=mycursor.fetchall()
...
#   mycursor.execute("""insert into `dailyreservation` (`TableId`, `Slot`) values ('{}', '{}')""".format(reservedTable, reservedSlot))
#   mydb.commit()

#   return render_template('reservations.html', message="Reservation for {} at {} was successful".format(reservedTable, reservedSlot), slot_table={})
